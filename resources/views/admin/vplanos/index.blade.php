...
@section('content')
    ...
    <div>
        <h1>Visación de Planos</h1>        
    </div>
    <div class="card">
        <br>
        <div class="col-sm-12 col-md-6">
            <form action ="{{route('admin.vplanos.create')}}">
                <button type="submit" class="btn btn-outline-info btn-md">Crear visación</button>
            </form>
        </div>
        <div class="card-body">
            <table id="clientes" class="table table-striped table-bordered" style="width:100%">
                ...
                <tbody>
                    @foreach ($vplanos as $vplano)
                    <tr>
                        <td>{{$vplano->cliente->name}}</td>
                        <td>{{$vplano->name}}</td>
...
@section('js')
    ...
    @if (session('eliminar') == 'ok')
        <script>
            Swal.fire(
            'Eliminado',
            'La visación ha sido eliminado correctamente.',
            'success'
            )
        </script>
    @endif
    ...
    <script>
        $('.formulario-eliminar').submit(function(e){
            ...
            Swal.fire({
                title: '¿Estás seguro de eliminar la visación?',
                icon: 'warning',
